...
public class Free_Board_check implements CommandAction {

	@Override
	public String requestPro(HttpServletRequest request, HttpServletResponse response) throws Throwable {
int num = Integer.parseInt(request.getParameter("num"));
		
		BoardDao_Free dao1 = new BoardDao_Free();
		dao1.check(num);
		
		
		BoardDto_Free dto1 = dao1.getRead(num);
		
		// 포인트 업데이트 + 레벨 업데이트
				JsonFile2 jf = new JsonFile2();
				JSONObject json = jf.readJsonFile(request.getServletContext().getRealPath("upload")+"/point.json");
				new UserDao().updatePointAndLevel(true,dto1.getId(),"free_board_point",json);
				...
				msg.setMessage("Free게시판의 글이 채택되었습니다"); // 회원에게 보낼 메세지
