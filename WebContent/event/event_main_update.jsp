	...
	$(function() {
		$("input[id=cImg1]").change(function() {
			...
			$.ajax({
				...
				contentType : false,
				success : function(savefile) { // 업로드 성공하면 파일명을 받아온다.
					$('#contentImg').val(JSON.parse(savefile).savefile);
					$('#contentIMG').html("");
				},
				error : function(error) { // 실패할 경우 에러메세지 출력
					console.log(error);
				...
				.click(
						function() {
							...
							if (cate == 0) {
								...
								$('#alertContent').html("메인이미지를 선택해주세요.");
								$('#myModal10').modal('show');
								$("#mImg").focus();
								return;
							} else if ($("#contentImg").val() == "") {
								$('#alertContent').html("내용이미지를 선택해주세요.");
								$('#myModal10').modal('show');
								$("#cImg").focus();
								return;
							}
							...
							$.ajax({
								type : "POST",
								url : "event_main_updateAction.do",
								data : add,
			...
			<div class="col-lg-a">
				<div class="jumbotron" style="padding-top: 20px;">
					...
					<form method="post" id="form" action="event_main_writeAction.do">
						...
						<div class="form-group">작성자 : ${sessionScope.Id }</div>
						...
						<div class="oaerror warning">
							<span id="sub"></span><input type="text" id="subject" name="subject" value="${a.subject }">
						</div>
						<div id="discount" style="display: none;" class="oaerror info">할인율<input type="text" name="discount" id="discount1"></div>
						<div id="dcate1" class="oaerror danger">
						<span id="cate1"></span>
						<input type="hidden" id="cate2" name=""	placeholder="정수 값" />
						<input type="hidden" id="cate3" value=0>
						<input type="hidden" name="count" value="${a.count }">
						<input type="hidden" name="finish" value="${a.finish }">
						</div>
						<div class="oaerror warning">
							제한 레벨
							<input type="text" id="level" name="level" value="${a.level }"	placeholder="정수 값">
						</div>
						<input type="hidden" id="mainImg" name="mainImg" value="${a.imgMain }">
						<input type="hidden" id="contentImg" name="contentImg" value="${a.imgContent }">
						<div class="oaerror info">
						메인 이미지 선택 : <input type="file" name="mImg" id="mImg"style="display:inline;" />
						<span id="mainIMG">현재 메인 이미지 : ${a.imgMain }</span>
						</div>
						<div id="cImg" class="oaerror danger"> 내용 이미지 선택 : <input type="file" name="cImg" id="cImg1" style="display:inline;">
						<span id="contentIMG">현재 내용 이미지 : ${a.imgContent }</span></div>
						<div id="date" class="oaerror warning">기한 : <input	type="date"  name="date" id="date1" value="${a.date }"></div>
							<input type="button"class="btn btn-primary form-control" value="수정" name="ck" id="ck" >
